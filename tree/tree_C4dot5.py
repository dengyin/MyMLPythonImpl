...
def infoGainRadio(dataBeforeSplit, datasAfterSplitByFeature):
    ...
    for i in datasAfterSplitByFeature.keys():
        ...
        HaD -= prob * math.log(prob, 2)
    return (oldEnt - newEnt) / HaD
...
def findBestSplit(dataSet, classSet):
    ...
    for i in range(featureNum):
        if infoGainRadio(classSet, datasAfterSplitByFeature(dataSet, classSet, i)) > mostInfoGain:
            bestSplitFeature = i
            mostInfoGain = infoGainRadio(classSet, datasAfterSplitByFeature(dataSet, classSet, i))
...
def C4_5(dataSet, classSet, labelSet):
    ...
    for key in dataSet.keys():
        tree[bestFeatureLabel][key] = C4_5(dataSet[key][:, :-1], dataSet[key][:, -1], labelSet)
    return tree
